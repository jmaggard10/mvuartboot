 ...
 *
 * Based on kwuarboot (http://www.solinno.co.uk/public/kwuartboot/)
 * Copyright (C) 2011
...
static inline void putcharf(int c)
{
	putchar(c);
	fflush(stdout);
}
...
int
send_boot_pattern(int fd, int timeout, int stage)
{
	int wrote;
	int c;
	int done;
	...
	if (stage == 0)
		...
		printf("Re-sending boot pattern: please wait...");
...
xmodem_send(int tty_fd, int in_fd)
{
	...
	struct stat st;
	int packetno, lastpacket, packets;
	int c;
	int retry;
	int res;
	...
	if (fstat(in_fd, &st) != 0)
		return -1;
	packets = st.st_size / 128;

	/* We have already received a NAK, but wait for another to be sure */
	if (wait_for_nak(tty_fd) == -1)
		return -1;

	/* Some machines (Armada XP) require us to do this a second time */
	res = send_boot_pattern(tty_fd, PATTERN_SEND_TIMEOUT, 1);
	wait_for_nak(tty_fd);

	lastpacket = 0;
	packetno   = 1;
	for (retry = 0; retry < MAX_RETRANS; ++retry) {
		printf("\rSending file... %02d%%", packetno * 100 / packets);
 ...
 next:
		...
		default:
			putcharf(c);
			c = read_byte(tty_fd, RECEIVE_TIMEOUT);
			goto next;
		case NAK:
			break;
		}
...
main(int argc, char *argv[])
{
	...
	res = send_boot_pattern(tty_fd, PATTERN_SEND_TIMEOUT, 0);
	switch (res) {
	case 0:
		res = xmodem_send(tty_fd, file_fd);
		break;
	case -1:
	...
	for (;; ) {
		char c = read_byte(tty_fd, PATTERN_SEND_INTERVAL);
		if (c != -1 && (isprint(c) || iscntrl(c)))
			putcharf(c);
		c = read_byte(STDIN_FILENO, 0);
		if (c != -1 && (isprint(c) || iscntrl(c))) {
			if (c == 0x1c)
				ctrl = 1;
			else if (ctrl) {
